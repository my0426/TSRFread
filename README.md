...
> 📊 柴油机一维热力学模型结构图
![柴油机一维热力学模型结构图](assets/fig1.jpg)
*该图展示了论文搭建的船舶柴油机一维仿真模型，包含涡轮增压器（TC1）、中冷器（CO1）、6个气缸（C1-C6）及进排气歧管（PL1/PL2）。模型通过阀门（MP1-MP6）控制流路，可模拟真实柴油机的进气-燃烧-排气过程，是后续故障参数微调、特征采集的核心载体。*
...
> 📊 基于 SHAP 值的参数重要性分析图
![基于 SHAP 值的参数重要性分析图](assets/fig2.jpg)
*该组图从3个角度分析参数对故障诊断的贡献：1. 热力图（a）：展示不同参数在各故障下的SHAP值（贡献度），P14、P05在多数故障中贡献度较高；2. 平均SHAP值图（b）：按贡献度排序参数，明确P14、P05、P06为核心诊断参数；3. 占比图（c）：体现参数与故障类型的关联分布，为后续特征筛选提供依据。*
...
![基于 SHAP 值的活塞环磨损故障分析图](assets/fig3.jpg)
*该组图以F4（活塞环磨损）为例，实现模型诊断过程的可解释性：1. 水滴图（a）：解析单个F4样本的诊断逻辑，展示各参数的正负贡献；2. 蜂群图（b）：展示所有样本中参数对F4的贡献分布，P12、P14是核心影响参数；3. 交互图（c）：体现参数间的交互作用（如P11与P12）；4. 依赖图（d）：明确参数取值与SHAP值的关系，为故障机理分析提供依据。*
...
> 📊 混淆矩阵图
![混淆矩阵图](assets/fig4.jpg)
*该组混淆矩阵对比了不同方法的分类性能。图(a)-(e)：传统模型（如KNN、SVM）或原始特征集的结果，存在较多错分（如F3、F4样本误判）；图(f)：TSRF方法（SHAP筛选特征+随机森林）的结果，仅极少量样本错分，验证了方法的高分类准确率。*

> 📊 精确率 - 召回率曲线
![精确率 - 召回率曲线](assets/fig5.jpg)
*该组曲线从分类阈值维度验证性能。图(a)-(e)：传统模型/特征集的曲线中，部分故障（如F3）的AUC值较低；图(f)：TSRF方法的曲线中，所有故障的AUC接近1.0，整体Micro-average与Roc-AUC达1.0，体现了模型的稳定高性能。*
